...
TOKEN = 'adawsrsaedtseredrgtedsrft'
...
def check_data():
    ...
    def modification_date():
        t = os.path.getmtime('../rtd/EURUSD.json')
        return dt.datetime.fromtimestamp(t)+dt.timedelta(seconds=300)<dt.datetime.now()
    if modification_date() and telegram_time + dt.timedelta(seconds=60) < dt.datetime.now():
        mesg='Data disconnected '+str(dt.datetime.now())
        url="https://api.telegram.org/bot1362017119:AAElZH6CNiQIXBKkTkfOSqiz6i-4UeV-Fok/sendMessage?chat_id=-408108573&text="+str(mesg)
        #requests.get(url)
        try:
            con.close()
        except:
        ...
        test = subprocess.Popen(["python","data.py"], stdout=subprocess.PIPE)
